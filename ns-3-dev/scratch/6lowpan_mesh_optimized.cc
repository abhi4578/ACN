...
// Mesh topology with partially optimized ND
#include <fstream>
...
SixLowPanHelper sixlowpan;
 NetDeviceContainer sixlowpandevices = sixlowpan.Install (lrwpandevices); 
  ...
  radvdHelper.GetRadvdInterface (net.GetInterfaceIndex (4))->SetSendAdvert (false); 


  ApplicationContainer radvdApps = radvdHelper.Install (br.Get(0));
  radvdApps.Start (Seconds (1.0)); 
  radvdApps.Stop (Seconds (3600.0));
  lrWpanHelper.EnablePcapAll (std::string ("mesh-topology-optimized-frames"), true);
  internetv6.EnablePcapIpv6All(std::string ("mesh-topology-optimized-ip"));
